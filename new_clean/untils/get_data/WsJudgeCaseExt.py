"""
@author : kennethAsher
@fole   : WsJudgeCaseExt.py
@ctime  : 2020/5/19 14:38
@Email  : 1131771202@qq.com
@content: 从原始数据中拿到ws_judgecase_ext所需要的数据
            id, judge_name, judge_status(职位),person_cnt(当事人数量), company_cnt(当事人公司数量),court,courtlevel,docid,casereason,casetype(案件类型),
            doctype,trialprocedure(审理程序),judgeyear,judgemonth,judgetime,伙伴名称，关联律师,當事人（姓名，角色），公司
"""
...
class WSJudgeCaseExt():
    def __init__(self):
        self.data_input_path = '/mnt/disk2/data/sum_data/add_result/'
        self.data_out_path = '/mnt/disk2/data/sum_data/result/ws_judgecase_ext/'
                    ...
                    for judge in judges:
                        ...
                        if len(parties)>0:
                            for party in parties:
                                if '个人' in party:
                                    person_cnt += 1
                                    persons = persons+','+party
                                else:
                                    company_cnt += 1
                                    companies = companies + ',' + party
                        ...
                        out_line = _id+'|'+judge_name+'|'+judge_status+'|'+str(person_cnt)+'|'+str(company_cnt)+'|'+court+'|'+court_level+'|'+doc_id+\
                                   '|'+cause_name+'|'+casetype+'|'+doc_type+'|'+trail+'|'+judge_year+'|'+judge_month+'|'+judge_date+'|'+lawyer+\
                                   '|'+judge_list+'|'+persons+'|'+companies+'\n'
