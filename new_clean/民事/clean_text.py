...
logging.basicConfig(filename='/Users/by-webqianduan/OneDrive/pycharm/bangying/log/clean_oc/民事/clean_text.log',
                    filemode="w",
...
# 法院判决
court_lawsuit_pat = re.compile(r'判决|裁决|裁定')
        ...
        for line in file_open.readlines():
            ...
            for line in lines:
                if '一案' in line:
                    read_flag = 1
                if read_flag > 0:
                    if plaintiff_lawsuit_pat.findall(line) is not None:
                        read_flag = 2
                    if appellee_lawsuit_pat.findall(line) is not None:
                        read_flag = 3
                    if court_end_pat.findall(line) is not None:
                        read_flag = 1
                    if court_lawsuit_pat.findall(line) is not None:
                        read_flag = 4
                    if read_flag == 2:
                        # print(line)
                        plaintiff_lawsuit += line
                    if read_flag == 3:
                        appellee_lawsuit += line
                    if read_flag == 4:
                        court_lawsuit += line
            # file_write.write(doc_num+'|'+plaintiff_lawsuit+'|'+appellee_lawsuit+'|'+court_lawsuit)
            # print(plaintiff_lawsuit)
        file_open.close()
...
if __name__ == '__main__':
    ...
    file_write = '/Users/by-webqianduan/data/lawsuit_text/lawsuit_data'
...
'''
#原告诉求判断
plaintiff_lawsuit_pat = re.compile(r'((原审原告|原告人|原告|上诉人|申诉人|再审申请人|申请人|申请执行人|异议人|起诉人|申报人|原审第三人|第三人)|一审法院).*(诉讼|请求|诉请|诉称)')
# 被告诉求
appellee_lawsuit_pat = re.compile(r'((原审被告|被告人|被告|被上诉人|被申诉人|申请再审人|被申请人|被执行人|被异议人)|一审法院).*(答辩|反诉|诉讼|请求|诉请|诉称|认为)')
# 法院认为中断
court_end_pat = re.compile(r'(本院认为|事实与理由)')
# 法院判决
...
def run(open_dir, write_dir):
    write_lawyer_judge(open_dir, write_dir)


if __name__ == '__main__':
    file_open = 'D:\\temp\\tu'
    file_write = 'D:\\temp\\out'
