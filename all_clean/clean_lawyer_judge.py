...
logging.basicConfig(filename='/mnt/disk1/log/python/{}/clean_lawyer_judge.log'.format(_type),
                    filemode="w",
...
def write_lawyer_judge(open_dir, write_dir, names):
    for step, name in enumerate(names):
        ...
        for line in file_open.readlines():
            ...
            #因为提取的时候将换行替换成了了'。'，所以在文书可能连续出现多个句号，此处需要将这些替换
            line_judge = line.replace('。。。。。', '').replace('。。。。', '').replace('。。。', '').replace('。。', '')
...
if __name__ == '__main__':
    ...
    file_write = '/mnt/{}/data/{}/lawyer_judge/'.format(_disk, _type)
    exists_or_create(file_write)
